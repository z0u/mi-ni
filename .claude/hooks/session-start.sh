...
# 1. Ensure uv is new enough to parse pyproject.toml — first, ahead of both the
#    orientation probe and the async handoff. The image ships uv 0.8.x, which
#    can't parse the relative `exclude-newer = "N days"` cooldown: it drops the
#    cutoff (and the rest of `[tool.uv]`, so a `required-version` guard there
#    can't catch it), re-resolves to newer packages, downloads them, and
#    rewrites uv.lock. Any `./go` or `uv run` in the window before the upgrade
#    lands did just that, so the upgrade has to finish first. It takes under a
#    second even on a cold cache, which is why it fits ahead of the map.
...
# 2. Print a compact orientation MAP synchronously, before the async handoff
...
(
    ...
    # so nothing here can trigger the sync that step 5 below defers. `./go` does
    # read `uv --version` for its guard, though, and on a cold container it
    # reported that guard failing as the whole tooling line — which is why the
    # upgrade in step 1 runs above this block rather than below it. The venv
...
# 6. Install the agent-friendly CLI tools the dev container ships as features
