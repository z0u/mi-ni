...
`mini.lit` is the prototype: a Markdown file whose ```` ```{python} ```` fences are cells run top to bottom in one namespace, prose rendered as Jinja against that namespace (interpolation, `{% for %}` tables, helper calls), the same pymdownx Markdown dialect as `mo.md`, a `@memo` cache keyed by `mini.memo`'s source and input fingerprints, `stop()` with pending marks for a preregistration, a watch-and-reload server, and Markdown, HTML and PDF outputs from one pass. Demos in `docs/lit/`; `./go lit render|serve`.

Open, in rough order:

- Decide the format. Markdown-with-fences puts prose first and gives the template loops for free; the cost is that editor tooling does not see inside a fence (no ruff, no ty, no jump-to-definition), which the helper-module pattern our reports already follow keeps small. A `lit check` that extracts the cells to a temp module for ruff and ty would close most of the gap. The alternative, Python-with-Markdown-strings, keeps IDE support and gives up prose-first editing and templating; the runner and cache do not care which, only `parse` does.
