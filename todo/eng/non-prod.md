...
**Setup as a skill.** The profile mechanism is library usage, so it goes in the `mi-ni` skill's `references/storage.md` beside `store-bucket`/`publish-repo`; the setup steps (create the two dev repos, add the table, mint a dev-only token for the engineering environments, point the test runner at the profile) go in a short `.agents/skills/storage-envs/SKILL.md` that the vault skill in `backups.md` can cross-link, since the two are set up together.
