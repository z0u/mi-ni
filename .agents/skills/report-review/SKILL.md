...
4. If the loop converged and the report is heading for a freeze or a publish, render the document to Markdown and hand that file to the `report-structure` agent:
   ...
   Hand it the render, not `report.py`: a cell can fail to render and leave no trace in the source, and we have had a report publish without one of its verdicts that way. Hand it `index.md` rather than `index.html`: the same document, without the page's stylesheet and markup. The render re-runs the script's cells (memoized work comes from the cache), so allow a few minutes. `report-render` has the details.
