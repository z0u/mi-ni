...
4. If the loop converged and the report is heading for a freeze or a publish, render the document to Markdown and hand that file to the `report-structure` agent:
   ...
   Hand it the render, not `report.py`: a cell can fail to render and leave no trace in the source, and we have had a report publish without one of its verdicts that way. And the Markdown render, not the bundle's `index.html`: same document, about ten times the size, most of it the report's framework markup. The render re-weaves the script, so allow a few minutes — unless the report hasn't been touched since its last render, which `./go render` skips (`--force` re-renders anyway). `report-render` has the details.
