...
- It ships your source automatically, adding the project's top-level `src/` packages via `add_local_python_source(*project_packages())`, so a remote worker can `import experiment...`/`import utils...` with no manual mounting or packaging. New top-level packages under `src/` are discovered automatically.
