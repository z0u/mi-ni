...
- Two asset dirs, no collision: the runtime lives under `assets/` (from `_static/`), the report's figures under `_assets/` (leading underscore, from the bundle). `render.py` symlinks both into the serve root.
- Chromium is pre-installed at `/opt/pw-browsers/chromium` (override with `PLAYWRIGHT_CHROMIUM`); don't run `playwright install`.
