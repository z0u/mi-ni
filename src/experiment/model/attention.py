...
class RotaryEncoding(nn.Module):
    def __init__(self, n_head_dim, base=10_000):
        super().__init__()
        inv_freq = 1.0 / \
            (base ** (torch.arange(0, n_head_dim, 2).float() / n_head_dim))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x)
        q, k, v = qkv.split(
            [self.n_kq_tot, self.n_kq_tot, self.n_v_tot], dim=-1)
        ...
        att = att * self.scale
        att = att.masked_fill(torch.tril(torch.ones(
            T, T, device=att.device)) == 0, float('-inf'))
