...
nGPT keeps that spirit but reframes it geometrically. The step is a _spherical LERP_: interpolate a fraction $\alpha$ of the way from the current hidden state towards the sub-module's **normalized** output, then re-project onto the sphere:

$$h \leftarrow \text{Norm}\big(h + \alpha\,(\text{Norm}(\text{sub}(h)) - h)\big)$$

Normalizing $\text{sub}(h)$ before the step is the load-bearing detail, even though it looks like a nicety. It makes $\alpha$ a true interpolation fraction, so the per-layer rotation is $\approx\alpha$ regardless of scale. The gate $\alpha$ is a single learnable scalar per sub-module (a ReZero/LayerScale-style gate, initialised at 0.05); the [full recipe](../../../docs/gpt-sweep/report.py) uses a per-**channel** vector instead (the paper's _eigen learning rates_), and the sweep shows the scalar simplification is free at this scale.

It is tempting to skip the inner normalization and just step towards the raw output, $h \leftarrow \text{Norm}\big(h + \alpha\,\text{sub}(h)\big)$ — and at small width it trains fine, so we shipped it that way at first. It's a trap. $\text{sub}(h)$ is _not_ unit-norm: the MLP scales its pre-activations by a $\sqrt{n_\text{embd}}$ baseline (above), so $\lVert\text{MLP}(h)\rVert \propto \sqrt{n_\text{embd}}$. The effective per-layer rotation is then $\alpha\,\lVert\text{sub}(h)\rVert$, which **grows with width**, so a _fixed_ $\alpha$ can't control it and the model destabilizes wide-and-deep (width 128, twelve layers).

What masks the trap is that our $\alpha$ is _learnable_ and reparametrized by $\sqrt{n_\text{embd}}$ (see `Scale`): it shrinks as width grows and quietly cancels the $\lVert\text{sub}(h)\rVert$ term — so with the learnable gate the additive step still trains, which is exactly why the bug hid. But that is the gate laundering a geometry error rather than a correct residual. Normalizing $\text{sub}(h)$ removes the error at the source, making $\alpha$ a true interpolation fraction independent of the gate. The [sweep report](../../../docs/gpt-sweep/report.py) has the three-way width sweep — fixed-gate additive (fails), learnable-gate additive (masked), normalized LERP (fixed) — and adopts the normalized LERP as the default; the additive forms survive as `config.normalize_sublayer=False` (with `learnable_alpha=False` for the fixed gate) to reproduce the failure. The learned gates settle near $1/n_\text{layer}$ on average — the value that keeps the twelve-layer stack moving an $O(1)$ distance overall.
