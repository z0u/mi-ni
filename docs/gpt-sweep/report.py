# title: Architecture sweep: GPT versus nGPT
...
_flat = {(a, lr): v for a in ARCHS for lr in LRS if (v := curves.get(f"{a}|{lr}"))}
_best_arch, _best_lr = min(_flat, key=lambda k: min(_flat[k]))

rf"""
**Best run:** {_best_arch} at peak LR {_best_lr} — val_loss
**{min(_flat[(_best_arch, _best_lr)]):.2f}** over {len(_flat)} architecture×LR cells.
...
if not any(f"width|fixed|{w}" in curves for w in WIDTHS):
    stop("No width-sweep results yet.")

_fixed_128 = plateau(curves, "width|fixed|128")
_norm_128 = plateau(curves, "width|norm|128")
_norm_spread = max(plateau(curves, f"width|norm|{w}") for w in WIDTHS) - min(
    plateau(curves, f"width|norm|{w}") for w in WIDTHS
)

rf"""
**A fixed gate exposes the bug; the fix removes it.** With a fixed step size the additive
residual falls apart as width grows — converged loss climbs to **{_fixed_128:.2f}** nats/char
at width 128. The normalized-LERP fix reaches **{_norm_128:.2f}** there, flat across all three
widths (spread just {_norm_spread:.02f} nats/char). Same learning rate, same everything else;
...
def width_gating_figure() -> plt.Figure:
    colors = resid_colors()
